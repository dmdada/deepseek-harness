...
- **Session Delete has no confirmation dialog** — the Archived row's Delete commits directly (the archive posture); a destructive confirm is deferred. The Archived section itself renders only in the grouped tree, not the flat "In one list" mode.
- **Workspace registration deletion does not delete Sessions** — removing a Workspace only drops its accounting; the Sessions fall back under Ungrouped.
