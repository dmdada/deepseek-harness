...
- **「删除会话」没有确认对话框**：已归档行的 Delete 直接提交（沿用归档姿势）；破坏性确认暂缓实现。「已归档」区段只在分组树中渲染，平铺的「单一列表」模式不显示。
- **删除 Workspace 注册记录不会删除 Session**：仅移除其记账，Session 落入 Ungrouped 下。
