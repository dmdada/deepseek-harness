...
- **Session 删除没有确认对话框**：已归档行的 Delete 直接提交（沿袭归档的做法）；破坏性确认被延期。已归档分区本身只在分组树中渲染，在扁平的单列表模式下不渲染。
- **删除 Workspace 注册记录不会删除 Session**：移除 Workspace 只丢弃其记账；这些 Session 落回 Ungrouped。
