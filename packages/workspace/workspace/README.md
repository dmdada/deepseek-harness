...
- `ctx.workspaceRegistry.archiveSession(id)` / `unarchiveSession(id)` / `deleteSession(id)` / `archivedSessionIds` — the registry-global archive set, layered over workspace accounting: an archived session disappears from grouping surfaces but keeps its session log and its `sessionIds` slot, so a future `unarchiveSession` restores its position. Archiving accepts any live or persisted session (accounted or Ungrouped), resolves without writing for an already archived id, and rejects an unknown id. `deleteSession(id)` is the destructive counterpart: it removes the archive entry, detaches the id from every workspace account, and deletes the persisted log, while `/archivedSessionIds` itself is a read-only view. State written before the field existed parses with an empty set.
...
- **Session deletion is now scoped to the log** — `deleteSession(id)` removes the archive entry, every workspace account, and the persisted log, but never the workspace directory or user files; destructive folder removal remains separate and absent ([decision](../../../.agents/notes/implemented/feature/2026-07-27-workspace-registration-deletion.md)).
- **Archiving stays non-destructive** — an archived session is hidden without deleting its log, so `unarchiveSession` restores it.
