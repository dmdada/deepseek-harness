...
- **Removal never deletes user data** — removing a project leaves its folder and files in place; those sessions become ungrouped. Session deletion is scoped to the log: `deleteSession(id)` removes the archive entry, every workspace account, the live session entry, and the persisted log, but never the workspace directory or user files; destructive folder removal remains separate and absent ([decision](../../../.agents/notes/implemented/feature/2026-07-27-workspace-registration-deletion.md)).
- **A live session with no release capability is only drained** — deletion retires an attached idle session through `ctx.agents.release`, which needs the agent factory that produced the entry. A session entered without that capability (no agent registry mounted, or an entry registered outside the factory) is flushed instead, and its live row stays on the session list until its owner disposes it.
...
- **Archiving stays non-destructive** — an archived session is hidden without deleting its log, so `unarchiveSession` restores it to every grouping surface.
