...
- `ctx.workspaceRegistry.archiveSession(id)` / `unarchiveSession(id)` / `deleteSession(id)` / `archivedSessionIds`：覆盖在 workspace 记账之上的注册表级全局归档集合：被归档的会话从各分组视图中消失，但其会话日志和 `sessionIds` 席位保持不变，未来调用 `unarchiveSession` 可恢复原位置。归档接受任何实时或已持久化的会话（无论已记账还是 Ungrouped），对已归档的 id 直接完成而不写入，并拒绝未知 id。`deleteSession(id)` 是破坏性对应操作：它移除归档条目、把该 id 从每个 workspace 记账中分离，并删除持久化日志；`archivedSessionIds` 本身是只读视图。在该字段出现之前写入的状态解析为一个空集合。
...
- **会话删除现在限定于日志**——`deleteSession(id)` 移除归档条目、每个 workspace 记账以及持久化日志，但绝不触碰 workspace 目录或用户文件；破坏性文件夹移除仍然是独立且尚未提供的功能（参见[决策记录](../../../.agents/notes/implemented/feature/2026-07-27-workspace-registration-deletion.zh.md)）。
- **归档保持非破坏性**——被归档的会话只是被隐藏而不删除其日志，因此 `unarchiveSession` 可恢复它。
