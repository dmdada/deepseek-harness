...
`delete(id)` 是归档的破坏性对应操作：它移除被寻址会话的持久产物并释放该 id 供再次使用，而归档的会话保留自己的日志，以便之后 `open` 恢复它。不存在的 id——从未实体化的会话，或已被删除的会话——按无操作处理并正常完成。调用方需先确认没有任何活跃写句柄持有该 id，因为活跃持有者存在时删除会与其写回竞争；无法移除已存储会话的后端会拒绝继承的默认实现，而不是自行发明一种更弱的移除方式。
...
- **删除由调用方驱动，没有保留策略**——`delete` 只移除被寻址的那一个会话及其产物；没有任何机制决定会话何时该被淘汰，而没有移除路径的后端会拒绝默认实现。
