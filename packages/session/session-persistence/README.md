...
await ctx.sessionPersistence.delete(id)                        // remove one stored session and its artifacts; an absent id is a no-op
```
...
`delete(id)` is the destructive counterpart to archiving: it removes the addressed session's durable artifacts and releases the id for reuse, while an archived session keeps its log so a later `open` restores it. An absent id — a session that never materialized, or one already deleted — resolves as a no-op. The caller establishes first that no live write handle owns the id, because deleting under an active owner races that owner's write-behind; a backend that cannot remove stored sessions rejects the inherited default instead of inventing a weaker removal.
...
- **Deletion is caller-driven, with no retention policy** — `delete` removes the one addressed session and its artifacts; nothing decides when a session has aged out, and a backend without a removal path rejects the default.
