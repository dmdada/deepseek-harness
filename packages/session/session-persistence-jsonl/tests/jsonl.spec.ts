...
describe('JsonlSessionPersistence: session deletion', () => {
  ...
  it('removes the whole session-owned directory and leaves the id absent', async () => {
    const m = meta('deleted', '/project')
    await writeLog(ctx.sessionPersistence, m, oneTurnLog())
    const dir = sessionDir(root, m.cwd, m.id)
    // A non-log companion in the same directory proves delete removes the
    // session's whole artifact set, not just the log file it located.
    await writeFile(join(dir, 'metadata.json'), '{}\n')
    ...
    expect(await ctx.sessionPersistence.stat(m.id)).toBeUndefined()
    expect((await ctx.sessionPersistence.list()).map(snapshot => snapshot.header.id)).not.toContain(m.id)
    await expect(ctx.sessionPersistence.open(m.id, 'read')).rejects.toThrow(/not found/)
  })

  it('removes only the addressed session', async () => {
    const kept = meta('kept', '/project')
    const dropped = meta('dropped', '/project')
    await writeLog(ctx.sessionPersistence, kept, oneTurnLog())
    await writeLog(ctx.sessionPersistence, dropped, oneTurnLog())

    await ctx.sessionPersistence.delete(dropped.id)

    expect((await ctx.sessionPersistence.list()).map(snapshot => snapshot.header.id)).toEqual([kept.id])
    expect((await readAll(ctx.sessionPersistence, kept.id)).events).toEqual(oneTurnLog())
  })

  it('resolves an absent id as a no-op without creating storage', async () => {
    await expect(ctx.sessionPersistence.delete(SessionId('never-existed'))).resolves.toBeUndefined()

    // A created-but-never-materialized session owns no artifact, so deleting it
    // removes nothing and must not leave an empty session directory behind.
    const pending = await ctx.sessionPersistence.create(meta('never-materialized'))
    await expect(ctx.sessionPersistence.delete(pending.id)).resolves.toBeUndefined()
    expect(await readdir(root)).toEqual([])
    await pending.close()
  })

  it('observes cancellation before removing anything', async () => {
    const m = meta('aborted', '/project')
    await writeLog(ctx.sessionPersistence, m, oneTurnLog())
    const controller = new AbortController()
    controller.abort(new Error('delete cancelled'))

    await expect(ctx.sessionPersistence.delete(m.id, { signal: controller.signal }))
      .rejects.toThrow('delete cancelled')
    expect((await ctx.sessionPersistence.list()).map(snapshot => snapshot.header.id)).toContain(m.id)
  })
})
