...
- **Deleting a session removes its directory durably** — `delete(id)` locates the session's log, removes the session-owned directory, and syncs the surviving project directory; deleting an absent id is a no-op. Session logs no longer require external removal, so `root` does not accumulate abandoned session data.
