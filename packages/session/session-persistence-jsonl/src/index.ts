...
class JsonlSessionPersistence extends SessionPersistence {
  ...
  /**
   * Permanently remove one stored session: locate its unique generation across
   * every project directory and remove that session's whole directory, so the
   * log and every other file the session owns under it go together. An absent
   * id resolves as a no-op — a session that never materialized left no
   * directory to remove.
  ...
  override async delete(id: SessionId, options?: SessionPersistenceDeleteOptions): Promise<void> {
    ...
    options?.signal?.throwIfAborted()
    /* v8 ignore next -- Windows cannot fsync a directory handle; its namespace operations carry the removal. */
