...
The menu, Modal, and buttons retain their existing structure and design tokens. Session deletion remains visual-only and outside this decision; a later note gives Session deletion its own lifecycle ([Delete and unarchive archived sessions](2026-08-10-delete-and-unarchive-archived-sessions.md)).

## Alternatives considered

**Cascade-delete Sessions.** Rejected because Workspace registration does not own Session persistence and the product requirement is to preserve histories under Ungrouped. Session deletion now has its own lifecycle, running checks, descendant semantics, and explicit UI ([Delete and unarchive archived sessions](2026-08-10-delete-and-unarchive-archived-sessions.md)), kept distinct from registration deletion.
