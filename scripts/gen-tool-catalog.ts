...
// Local-only package: the generated alias region of `tsconfig.base.json` has no
// entry for its bare specifier, so its source is imported directly (the same
// reason the `tool-subagent/list-models` subpath below is imported by path).
import * as ToolImageVlm from '../packages/extensions/tool-image-vlm/src/index.ts'
...
const TOOL_PACKAGES: ToolPackage[] = [
  ...
  {
    pkg: '@deepseek-ai/dsh-tool-image-vlm',
    dir: 'tool-image-vlm',
    source: 'packages/extensions/tool-image-vlm/src/index.ts',
    requires: ['ctx.tools', 'ctx.shell', 'ctx.fs'],
    writes: ['tool/call', 'tool/result', 'one temporary downscaled JPEG under /tmp after a successful downscale'],
    async mount(ctx) {
      // The tool injects `shell` and `fs`; neither schema depends on which
      // providers back those seams, and none of them registers a tool here.
      await ctx.plugin(LocalSubprocessRuntime)
      await ctx.plugin(LocalBashExecutor)
      await ctx.plugin(LocalFileSystem)
      await ctx.plugin(ToolImageVlm)
    },
    note:
      'The image bytes leave the host for the OpenAI-compatible relay named by `relayUrl`, so the relay answers for a routed model that cannot accept image input; `read_image` from `@deepseek-ai/dsh-tool-fs` is the alternative under a route that does accept images. Both helper commands run through `ctx.shell` — the `sips` downscale exists only on macOS and falls back to the original file elsewhere — and the POST inherits the executor\'s sandbox, so a shell that denies outbound network access fails every call after downscaling.',
  },
  {
