...
| `@deepseek-ai/dsh-tool-image-vlm` | `image_understand` | `ctx.tools`、`ctx.shell`、`ctx.fs` | `tool/call`、`tool/result`、`one temporary downscaled JPEG under /tmp after a successful downscale` | - | 图片字节会离开主机，送往 `relayUrl` 指定的 OpenAI 兼容中转站，因此由中转站替无法接收图片输入的被路由模型作答；当路由确实接受图片时，替代方案是 `@deepseek-ai/dsh-tool-fs` 的 `read_image`。两条辅助命令都经由 `ctx.shell` 运行——`sips` 缩小仅在 macOS 上存在，其他平台回退为原文件——并且该 POST 继承执行器的沙箱，因此拒绝出网访问的 shell 会让每次调用在缩小之后失败。 |
...
<a id="deepseek-aidsh-tool-image-vlm"></a>

## `@deepseek-ai/dsh-tool-image-vlm`

### `image_understand`

用视觉语言模型感知并理解图片文件。把本地图片路径（可附带一个问题）发给中转站 VLM，取回文字描述或答案。凡需要看到图片内容时都用它：描述图片、读取文字或截图，或回答关于图片的问题。图片会先缩小到最长边 1024px 并转为 JPEG，然后才上传。
...
来源：[`packages/extensions/tool-image-vlm/src/index.ts`](../packages/extensions/tool-image-vlm/src/index.ts)

图片字节会离开主机，送往 `relayUrl` 指定的 OpenAI 兼容中转站，因此由中转站替无法接收图片输入的被路由模型作答；当路由确实接受图片时，替代方案是 `@deepseek-ai/dsh-tool-fs` 的 `read_image`。两条辅助命令都经由 `ctx.shell` 运行——`sips` 缩小仅在 macOS 上存在，其他平台回退为原文件——并且该 POST 继承执行器的沙箱，因此拒绝出网访问的 shell 会让每次调用在缩小之后失败。
